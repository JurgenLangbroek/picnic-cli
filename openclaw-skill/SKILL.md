...
homepage: https://github.com/jurgenlangbroek/picnic
metadata: {"openclaw":{"emoji":"🛒","requires":{"bins":["picnic"]},"install":[{"id":"npm","kind":"npm","package":"@jurgenlangbroek/picnic-cli","bins":["picnic"],"label":"Install picnic-cli (npm)"}]}}
env:
  PICNIC_API_URL: http://picnic-api:3000
---
...
- **Before any commands**, run `picnic status`. If not authenticated, tell the user to re-authenticate the Picnic server from the host.
- **Always ask the user before**: adding/removing cart items, clearing the cart, booking delivery slots.
